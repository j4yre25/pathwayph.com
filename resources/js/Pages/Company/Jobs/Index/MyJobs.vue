...
const props = defineProps({
  // Accept paginator object OR plain array
  jobs: { type: [Array, Object], default: () => [] },
  sectors: Array,
  categories: Array,
...
const jobList = computed(() => {
  ...
  if (props.jobs && Array.isArray(props.jobs.data)) return props.jobs.data;
  return [];
});
...
<template>
  <div>
    ...
    <!-- Job listings -->
    <div v-if="jobList.length" class="divide-y divide-gray-300">
          ...
          <div class="flex-1 min-w-0 mb-3 md:mb-0 md:mr-4">
            <div class="flex items-center space-x-2">
              ...
              <div v-if="job.job_types?.length" class="flex items-center space-x-1">
                <span v-for="type in job.job_types"
                      :key="type.id"
            ...
            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500 mt-1">
              ...
              <div v-if="job.salary" class="flex items-center text-sm text-gray-500">
                ...
                <span>
                  ...
                  <template v-if="job.salary.salary_type">/{{ job.salary.salary_type }}</template>
                </span>
              </div>
