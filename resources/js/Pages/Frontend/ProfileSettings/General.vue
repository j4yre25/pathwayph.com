...
const profile = ref({
  fullName: `${pageProps.user?.graduate_first_name || ''} ${pageProps.user?.graduate_middle_initial || ''} ${pageProps.user?.graduate_last_name || ''}`.trim(),
  graduate_first_name: pageProps.user?.graduate_first_name || '',
  graduate_middle_initial: pageProps.user?.graduate_middle_initial || '',
  graduate_last_name: pageProps.user?.graduate_last_name || '',
  graduate_professional_title: pageProps.user?.graduate_professional_title || '',
  email: pageProps.user?.email || '',
  graduate_phone: pageProps.user?.contact_number || '',
  graduate_location: pageProps.user?.graduate_location || '',
  graduate_birthdate: pageProps.user?.dob ? new Date(pageProps.user.dob) : null,
  graduate_gender: pageProps.user?.gender || '',
  graduate_ethnicity: pageProps.user?.graduate_ethnicity || '',
  graduate_address: pageProps.user?.graduate_address || '',
  graduate_about_me: pageProps.user?.graduate_about_me || '',
  graduate_picture_url: pageProps.user?.profile_picture || 'path/to/default/image.jpg',
  // Fields from Register.vue
  graduate_school_graduated_from: pageProps.user?.graduate_school_graduated_from || '',
  graduate_program_completed: pageProps.user?.graduate_program_completed || '',
  graduate_year_graduated: pageProps.user?.graduate_year_graduated || '',
  // Social links and contact form settings
  linkedin_url: pageProps.user?.linkedin_url || '',
  github_url: pageProps.user?.github_url || '',
  personal_website: pageProps.user?.personal_website || '',
  other_social_links: pageProps.user?.other_social_links || '',
  enable_contact_form: pageProps.user?.enable_contact_form || false,
});
...
const saveProfile = () => {
  ...
  settingsForm.post(route('profile.updateProfile'), {
    onSuccess: (response) => {
      profile.value.graduate_first_name = settingsForm.graduate_first_name;
      profile.value.graduate_middle_initial = settingsForm.graduate_middle_initial;
      profile.value.graduate_last_name = settingsForm.graduate_last_name;
      ...
      profile.value.fullName = `${profile.value.graduate_first_name} ${profile.value.graduate_middle_initial} ${profile.value.graduate_last_name}`.trim();
...
const validateForm = () => {
  ...
  if (!profile.value.graduate_first_name) {
    errors.graduate_first_name = 'First name is required';
    isValid = false;
  }
  
  if (!profile.value.graduate_last_name) {
    errors.graduate_last_name = 'Last name is required';
    isValid = false;
  }
    ...
    <Modal :show="isNoChangesModalOpen" @close="isNoChangesModalOpen = false">
      <div class="p-6">
        ...
        <div class="mt-6 flex justify-center">
          <button
            type="button"
            class="bg-blue-500 hover:bg-blue-600 transition-colors text-white px-4 py-2 rounded-md"
          ...
          <div class="space-y-4">
            ...
            <div class="relative">
              <label class="flex items-center">
                <input 
                  type="checkbox" 
                  v-model="profile.enable_contact_form"
