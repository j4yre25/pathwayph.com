...
const addEducation = () => {
  ...
  if (
    ...
    education.value.graduate_education_program.trim() === '' ||
    !education.value.graduate_education_field_of_study ||
    education.value.graduate_education_field_of_study.trim() === '' ||
    !education.value.graduate_education_start_date
  ) {
  ...
  // Validate start date
  const startDate = education.value.graduate_education_start_date instanceof Date 
    ? education.value.graduate_education_start_date 
    : new Date(education.value.graduate_education_start_date);
  ...
  if (!education.value.is_current && education.value.graduate_education_end_date) {
    const endDate = education.value.graduate_education_end_date instanceof Date
      ? education.value.graduate_education_end_date
      : new Date(education.value.graduate_education_end_date);
  ...
  const formattedStartDate = formatDate(education.value.graduate_education_start_date);
  const formattedEndDate = education.value.is_current ? null : formatDate(education.value.graduate_education_end_date);

  const educationForm = useForm({
    graduate_education_institution_id: education.value.graduate_education_institution_id.trim(),
    graduate_education_program: education.value.graduate_education_program.trim(),
    graduate_education_field_of_study: education.value.graduate_education_field_of_study.trim(),
    graduate_education_start_date: formattedStartDate,
    graduate_education_end_date: formattedEndDate,
...
<template>
    <div v-if="activeSection === 'education'" class="flex flex-col">
                  ...
                  <div v-for="entry in educationEntries" :key="entry.id" class="bg-white p-8 rounded-lg shadow relative">
                    <div>
                      ...
                      <div class="flex items-center text-gray-600 mt-2">
                        ...
                        <span>
                          {{ formatDisplayDate(entry.graduate_education_start_date) }} - {{ entry.graduate_education_end_date ?
                            formatDisplayDate(entry.graduate_education_end_date) : 'present' }}
                        </span>
                      </div>
                      <p class="mt-2">
                        <strong>
        ...
        <div v-if="showArchivedEducation" class="mt-8">
            ...
            <div v-if="archivedEducationEntries && archivedEducationEntries.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div v-for="education in archivedEducationEntries" :key="education.id" class="bg-gray-50 p-8 rounded-lg shadow relative border border-gray-200">
                    <div class="opacity-75">
                        <div class="border-b pb-2">
                            <h2 class="text-xl font-bold">{{ education.graduate_education_institution_id || 'Unknown Institution' }}</h2>
                            <p class="text-gray-600">
                                {{ education.graduate_education_program || 'Unknown Program' }} in {{ education.graduate_education_field_of_study || 'Unknown Field' }}
                            </p>
                        </div>
                        <div class="flex items-center text-gray-600 mt-2">
                            <i class="far fa-calendar-alt mr-2"></i>
                            <span>
                                {{ formatDisplayDate(education.graduate_education_start_date) }} - {{ education.graduate_education_end_date ? formatDisplayDate(education.graduate_education_end_date) : 'present' }}
                            </span>
                        </div>
                        <div v-if="education.graduate_education_term" class="flex items-center text-gray-600 mt-2">
                            <i class="far fa-calendar-alt mr-2"></i>
                            <span>
                                {{ education.graduate_education_term }}
                            </span>
                        </div>
                    </div>
                    <div class="absolute top-8 right-4 flex space-x-4">
