  ...
  <AppLayout title="Degrees">
    <template #header>
      <div class="flex items-center">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">Manage degree entries and statuses</p>
        </div>
      </div>
    </template>
    ...
    <Container class="py-8">
      ...
      <div class="mb-8 max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <!-- Buttons (match SchoolYears style) -->
        <div class="flex flex-wrap gap-3 p-6 justify-center">
          <Link :href="route('school-years', { user: userId })"
            class="inline-flex items-center px-4 py-2 rounded-lg shadow-sm bg-white border border-gray-200 text-gray-700 hover:bg-blue-50 hover:border-blue-400 hover:text-blue-600 transition-all duration-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-br from-blue-100 to-blue-200 rounded-2xl p-6 relative overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-105">
          <div class="flex flex-col items-center text-center">
            ...
            <p class="text-2xl font-bold text-blue-900">{{ degrees.length }}</p>
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-100 to-green-200 rounded-2xl p-6 relative overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-105">
          <div class="flex flex-col items-center text-center">
            ...
            <h3 class="text-green-700 text-sm font-medium mb-2">Degree Types</h3>
            <p class="text-2xl font-bold text-green-900">{{ uniqueDegreeTypes.size }}</p>
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-purple-100 to-purple-200 rounded-2xl p-6 relative overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-105">
          <div class="flex flex-col items-center text-center">
            ...
            <h3 class="text-purple-700 text-sm font-medium mb-2">Active Degrees</h3>
            <p class="text-2xl font-bold text-purple-900">{{ degrees.length }}</p>
          </div>
        </div>
      </div>
      
      <!-- Main Content -->
      <!-- CTA Bar (match SchoolYears) -->
      <div class="bg-white rounded-2xl shadow-lg p-6 flex flex-wrap items-center justify-between gap-4 border border-gray-100 mb-8">
        <div class="flex items-center space-x-4">
          <button
            @click="$inertia.get(route('degrees.list', { user: page.props.auth.user.id }))"
            :class="[
              route().current('degrees.list')
          ...
          <button
            @click="$inertia.get(route('degrees.archivedlist', { user: page.props.auth.user.id }))"
            :class="[
              route().current('degrees.archivedlist')
                ? 'bg-blue-50 text-blue-600 border-blue-200'
                : 'text-gray-600 hover:bg-gray-50 border-gray-200',
              'px-4 py-2 rounded-md flex items-center space-x-2 font-medium transition-colors border',
            ]"
          >
            ...
            <span>Archived Degrees</span>
          </button>
        </div>
        <div class="flex items-center space-x-3">
          <button
            @click="$inertia.get(route('degrees.create', { user: page.props.auth.user.id }))"
      ...
      <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 flex items-center justify-between border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
          <div class="flex items-center">
            ...
            <div>
              <h3 class="text-xl font-bold text-gray-800">Degrees</h3>
              <span class="text-sm text-gray-600">{{ degrees.length }} total</span>
            </div>
          </div>
        </div>
