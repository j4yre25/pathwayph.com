...
const filteredGraduates = computed(() => {
  ...
  return graduates.filter((g) => {
    const matchProgram = selectedProgram.value
      ? String(g.program_id) === selectedProgram.value
      : true;
    const matchCareer = selectedCareer.value
      ? g.current_job_title === selectedCareer.value
      : true;
        ...
        <div class="flex flex-wrap justify-end gap-4">
          ...
          <div>
            ...
            <select v-model="selectedCareer" class="rounded-md border-gray-300 focus:border-green-500 focus:ring-green-500">
              <option value="">All</option>
              <option v-for="career in props.careerOpportunities" :key="career" :value="career">
                {{ career }}
              </option>
            </select>
          </div>
