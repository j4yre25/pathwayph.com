...
const unemploymentLineOption = computed(() => ({
    tooltip: { trigger: 'axis' },
    ...
    series: [
        ...
        },
        {
            ...
            type: 'line',
            data: unemploymentOverTime.value.map(d => d.employment_rate),
            itemStyle: { color: '#22c55e' }
        }
    ]
}))

const unemploymentBarOption = computed(() => ({
    tooltip: { trigger: 'axis' },
    legend: { data: ['Unemployed', 'Employed', 'Total'] },
    xAxis: { type: 'category', data: unemploymentOverTime.value.map(d => d.year) },
    yAxis: { type: 'value' },
    series: [
        {
            name: 'Unemployed',
            type: 'bar',
            data: unemploymentOverTime.value.map(d => d.unemployed),
            itemStyle: { color: '#ef4444' }
        },
        {
            name: 'Employed',
            type: 'bar',
            data: unemploymentOverTime.value.map(d => d.employed),
            itemStyle: { color: '#22c55e' }
        },
        {
            name: 'Total',
            type: 'bar',
            data: unemploymentOverTime.value.map(d => d.total),
            itemStyle: { color: '#3b82f6' }
        }
    ]
}))
...
const tabs = [
    ...
    { label: 'Demand-Supply Career Gap Map', value: 'gapmap' },
                                    ...
                                    <span class="text-red-700 font-semibold">
                                        {{
                                            (() => {
                                                ...
                                                const avgUnemployment = arr.reduce((sum, d) => sum + d.unemployment_rate, 0) /
                                                    (arr.length || 1);
                                                return avgUnemployment.toFixed(1) + '%';
                                            })()
                                        }}
                                    </span>
                                    ...
                                    <span class="text-green-700 font-semibold">
                                        {{
                                            (() => {
                                                const arr = unemploymentOverTime.value;
                                                if (!arr.length) return 'N/A';
                                                const avgEmployment = arr.reduce((sum, d) => sum + d.employment_rate, 0) /
                                                    (arr.length || 1);
                                                return avgEmployment.toFixed(1) + '%';
                                            })()
                                        }}
                                    </span>
                                ...
                                <span>
                                    {{
                                        (() => {
                                            const arr = unemploymentOverTime.value;
                                            if (!arr.length) return 'N/A';
                                            const max = arr.reduce((a, b) => a.unemployment_rate > b.unemployment_rate ? a : b);
                                            return `${max.year} (${max.unemployment_rate}%)`;
                                        })()
                                    }}
                                </span>
                            </div>
                                ...
                                <span>
                                    {{
                                        (() => {
                                            const arr = unemploymentOverTime.value;
                                            if (!arr.length) return 'N/A';
                                            const min = arr.reduce((a, b) => a.unemployment_rate < b.unemployment_rate ? a : b);
                                            return `${min.year} (${min.unemployment_rate}%)`;
                                        })()}} </span>
                            </div>
                            <div v-else>
                                <span class="text-gray-500">No summary data available.</span>
                            </div>
                        </div>
                    </div>
                </div>
