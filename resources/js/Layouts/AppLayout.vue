...
import UserVerificationModal from '@/Components/Peso/UserVerificationModal.vue'
...
async function openNotification(notif) {
  try {
    ...
    if (notif.user_id) {
      axios.get(route('admin.manage_users.api', notif.user_id)).then(res => {
        userToVerify.value = res.data.user
    ...
    // Use notif.data.url if present
    const url = notif.data?.url || notif.url || notif.body?.url;
    if (url) {
      // Remove domain if present, only use path
...
<template>
        ...
        <div class="flex justify-between h-16">
          <div class="flex ">
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex"
              ...
              <NavLink
                v-if="page.props.permissions.canManageApprovalGraduate && page.props.auth.user.is_approved && page.props.auth.user.role === 'institution'"
                :href="route('institutions.career-guidance')" :active="route().current('institutions.career-guidance')">
                Career Guidance
              </NavLink>
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
              ...
              <NavLink :href="route('peso.job-referrals.index')" v-if="page.props.auth.user.role === 'peso'" Categories
                    ...
                    <div v-if="showNotifications" class="absolute right-0 mt-2 w-80 bg-white shadow-lg rounded z-50">
                      <div v-if="localNotifications.length">
                        <div class="flex justify-between items-center px-3 pt-3 pb-1">
                          ...
                          <button class="text-[10px] text-indigo-600 hover:underline" @click.stop="markAllNotifications"
                            v-if="unreadDisplay > 0">Mark all read</button>
                        </div>
                              ...
                              <div class="flex-1 min-w-0">
                                ...
                                <div v-else>
                                  ...
                                  <div class="text-xs text-gray-600 mt-1 line-clamp-2">
                                    {{ notif.data?.message || notif.message || 'Notification' }}
                                  </div>
            ...
            <div class="ms-3 relative">
              <Dropdown align="right" width="48">
                ...
                <template #content>
                  ...
                  <DropdownLink v-if="page.props.auth.user.role === 'institution'" :href="route('institution.profile')">
                    Profile
                  </DropdownLink>
  ...
  <UserVerificationModal v-model:show="showVerificationModal" :user="userToVerify" />
