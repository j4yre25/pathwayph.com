...
const mappedGroupedSkills = computed(() => {
    const result = {};
    Object.entries(props.groupedSkills).forEach(([category, skills]) => {
        result[category] = skills.map(skill => ({
            ...skill,
...
console.log('groupedSkills:', props.groupedSkills); 
</script>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 transition-all duration-300 border border-indigo-50 hover:shadow-lg">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-2">
                <span v-if="skills && skills.length > 0"
                    class="text-xs text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 shadow-sm">
        ...
        <div class="transition-all duration-500 ease-in-out">
            <div v-if="!skills || skills.length === 0"
                class="text-gray-500 text-center py-8 text-sm bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-6 mt-4">
                <template v-for="(categorySkills, category) in mappedGroupedSkills" :key="category">
                    ...
                    <div class="md:col-span-2 mt-2 mb-4 first:mt-0">
                        <h4 class="font-medium text-gray-700 border-b border-indigo-100 pb-2 flex items-center">
                            ...
                            {{ category }}
                            <span
                                class="ml-2 text-xs text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
                    ...
                    <div v-for="skill in showAllSkills ? categorySkills : categorySkills.slice(0, SKILLS_LIMIT)"
                        :key="skill.id"
                        class="space-y-3 group transition-all duration-300 hover:bg-indigo-50 p-6 min-h-[120px] rounded-lg border border-gray-100 hover:border-indigo-200 hover:shadow-md relative transform hover:-translate-y-1 mb-4">
                        ...
                        <div class="h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                            <div class="h-full rounded-full transition-all duration-500 transform group-hover:scale-x-105 relative overflow-hidden"
                                :class="{
                        ...
                        <span
                            class="block mt-2 text-indigo-700 font-semibold bg-indigo-50 group-hover:bg-white px-3 py-1 rounded-full text-xs transition-colors duration-300 border border-indigo-100 shadow-sm w-max ml-0">
                        ...
                        <div v-if="skill.years_experience"
                            ...
                            {{ skill.years_experience }} {{ skill.years_experience === 1 ? 'year' : 'years' }} of
                            experience
                        </div>
