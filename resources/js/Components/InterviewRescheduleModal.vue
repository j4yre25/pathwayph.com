<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white rounded-lg shadow w-full max-w-md p-6">
      <h3 class="text-lg font-semibold mb-4">Reschedule Interview</h3>
      <div class="space-y-3 text-sm">
        ...
        <div>
          ...
          <textarea v-model="reason" rows="2" class="mt-1 w-full border rounded px-2 py-1 text-sm"></textarea>
        </div>
        <div>
...
watch(() => props.show, v => {
  if (v) {
    message.value = 'We apologize, but your interview has been rescheduled to [new date/time]. Please confirm.'
...
function validate() {
  if (!date.value || !time.value) return 'New date & time required.'
...
async function submit() {
  ...
  sending.value = true
  try {
    await axios.post(route('interview.reschedule'), {
